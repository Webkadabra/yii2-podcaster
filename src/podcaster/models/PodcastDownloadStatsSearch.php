...
class PodcastDownloadStatsSearch extends PodcastStatListen
{
    public $searchQuery;
    /**
     * @inheritdoc
    ...
    public function rules()
    {
        return [
            [['episode_id'], 'integer'],
            [['searchQuery', /*'status', 'fulfillment_status', 'grand_total', */ 'time',], 'safe'],
...
//        $query->andFilterCompare('grand_total', $this->grand_total);
        if ($this->time) {
            if(strstr($this->time, ' -- ')) {
                list($start_date, $end_date) = explode(' -- ', $this->time);
                $query->andFilterWhere(['between', 'DATE(time)', $start_date, $end_date]);
            } else {
                $query->andFilterWhere(['DATE(time)' => date('Y-m-d', strtotime($this->time)),]);
            }
        }
        ...
        if ($searchQuery) {
            if (is_numeric($searchQuery)) {
                $query->andWhere([
                    "{$orderTable}.episode_id" => $searchQuery,
                ]);
//                $query->orWhere([
//                    "{$orderTable}.number" => $searchQuery,
//                ]);
                $query->orWhere([
                    "{$orderTable}.user_agent" => $searchQuery,
                ]);
//                $query->orFilterWhere([
