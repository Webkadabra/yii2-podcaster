...
count(DISTINCT ip_address, episode_id)  total_downalods
FROM  
podcast_stat_listen
...
order by date $order",
    ...
    public static function ddddataProvider($order='ASC') {
        $cutoff = date('Y-m-d', mktime(0,0,0,1,1, (date('Y') - 2)));
        $dataProvider = new SqlDataProvider(['sql' => "SELECT 
DATE_FORMAT(`time`, \"%Y-%m-%d\") `date`,
count(DISTINCT `podcast_stat_listen`.ip_address, user_agent)  total_downalods
/*count(DISTINCT `podcast_stat_listen`.id)  total_downalods*/
FROM  
podcast_stat_listen
join `podcast_episode` on podcast_stat_listen.episode_id = `podcast_episode`.id
/*WHERE 
